...
config='/home/user/.config/i3/autogen_keybindings'
genconf='/home/user/.config/i3/i3gen.conf'
vmlist=$(sed -E '/ALIAS[[:blank:]]+SYM[[:blank:]]+dom0_COMMAND/, $d' $genconf | sed '/^$/d' | grep -Ev "^#")
dom0_commands=$(sed -E '1,/ALIAS[[:blank:]]+SYM[[:blank:]]+dom0_COMMAND/d' $genconf | sed '/^$/d' | grep -Ev "^#")
qubeselect=$(grep -oe '^bindsym [^(\s|\t)]* mode "VM-Select"' $genconf)
wc=$(echo "$vmlist" | wc -l)

initialize_config() {
	# Verify the keybindings file is included in the main config 
	! grep -Eqs "include $config" /home/user/.config/i3/config \
		&& printf "%b" "# i3gen.sh autogenerated keybindings" >> /home/user/.config/i3/config \
		&& printf "%b" "include $config" >> /home/user/.config/i3/config

	# Overwrite old keybindings file 
	printf "%b" "####                   AUTO-GENERATED KEY BINDINGS                   ####\n" > "$config"
	printf "%b" "####         Edit i3gen.conf and run i3gen.sh to make changes        ####\n\n"  >> "$config"
}
